...
- `--tag` / `-t` — one of: `ux`, `ui`, `bug`, `copy`, `a11y`, `perf`, `nav`

If a flag value is not in the allowed list, do not guess or proceed — prompt the user to pick a valid value before writing.
...
- Tag: `ux`, `ui`, `bug`, `copy`, `a11y`, `perf`, `nav`
